...
$(function ($) {
	...
	{
		clearTimeout(timer);
	     timer = setTimeout(eval("bunnyIn"),"2000");
	    $("#puppy").css("left","-200").css("visibility","visible");
	    $("#puppy").animate({"top":50,"left":950},4000, "linear", null);
	}

	function puppyInTimer(event)
	{
	     clearTimeout(timer);
	     timer = setTimeout(eval("puppyIn"),"1500");
	 }

	 function bunnyIn(event)
	{
	    clearTimeout(timer);
	    timer = setTimeout(eval("girlPuppyIn"),"1000");
	    $("#bunny").css("left","-50").css("visibility","visible");
	    $("#bunny").animate({"top":10,"left":250},3000, "linear", null);
	}

	function bunnyInTimer(event)
	{
	     clearTimeout(timer);
	     timer = setTimeout(eval("bunnyIn"),"1100");
	 }
	 ...
	 function girlPuppyIn(event)
	{
	    ...
	    $("#girlPuppy").animate({"top":50,"right":750},3000, "linear", null);
	}
